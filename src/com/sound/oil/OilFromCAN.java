...
public class OilFromCAN {
	...
	public static double getOilByCANData(double h, int oilBoxType) throws Exception {
		double volume = -1;
		double height = (double)h /100;
		OilBox ob = OilBoxFactory.createOilBox(oilBoxType);
		volume = ob.getVolume(height);
		BigDecimal   b   =   new   BigDecimal(volume);  
		volume   =   b.setScale(2,   BigDecimal.ROUND_HALF_UP).doubleValue();  
		//System.out.printf("saving candata, oilBoxType into DB,  %.3f %.3f %d\n",height, volume, oilBoxType);
		return volume;
	}

	public static double getWaterByCANData(double h, int cartype) throws Exception {
		double volume = -1;
		double height = (double)h /100;
		OilBox ob = WaterBoxFactory.createWaterBox(cartype);
		volume = ob.getVolume(height);
		BigDecimal   b   =   new   BigDecimal(volume);  
		volume   =   b.setScale(2,   BigDecimal.ROUND_HALF_UP).doubleValue();  
		//System.out.printf("saving candata, oilBoxType into DB,  %.3f %.3f %d\n",height, volume, oilBoxType);
